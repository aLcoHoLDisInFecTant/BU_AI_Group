...
def run(config_path: str) -> None:
    ...
    # 以配置文件名作为标识，例如 bert_freeze、bert_formal、lstm_formal
    ...
    # Save results and checkpoint（使用 config_name 区分）
    root = os.path.dirname(SRC_DIR)
    result_path = os.path.join(root, "results", f"results_seed_{seed}_{config_name}.json")
    utils.save_json(history, result_path)

    ckpt_path = os.path.join(root, "models_checkpoints", f"model_seed_{seed}_{config_name}.pt")
